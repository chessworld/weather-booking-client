...
class OnboardingPage extends Component<RouteComponentProps, OnboardingPageState> {
    ...
    this.state = {
      ...
      pages: [
        <>
          ...
          <div className="onboarding-page-text">Do you have a special occasion coming up?</div>
          <div className="onboarding-page-image-container">
            <img src={Surfing} />
            <img src={FlyingKite} />
          </div>
        ...
        <>
          <div className="onboarding-page-text">Need a certain weather...?</div>
          <div className="onboarding-page-image-container">
            <img onTouchEnd={() => alert("Take yo to booking page")} src={BookButton} />
          </div>
          <div className="onboarding-page-text">The book it!</div>
        </>,
        <>
          <div className="onboarding-page-text">Buy us a coffee if we fullfill your weather needs</div>
          <div className="onboarding-page-image-container">
            <img onTouchEnd={() => alert("Take yo to coffee page")} src={Cup} />
          </div>
          <div className="onboarding-page-text">https://ko-fi.com</div>
        </>,
        <>
          <div className="onboarding-page-image-container">
          ...
          <div className="onboarding-page-text">
            <div className="onboarding-page-button" onTouchEnd={() => this.completeTutorial()}>
              Book Now
            </div>
          </div>
        </>,
  ...
  completeTutorial() {
    ...
    appCtx.setCompletedTutorial(true);
    UserEndpoint.completeUserTutorial(appCtx.userId).then(() => {
  ...
  handleScroll(e: SyntheticEvent): void {
    if (Object.keys(this.state).includes("lastTouchMousePositionX") && !this.state.hasMovedPagesThisTouch) {
      ...
      const currentValue: number = parseInt(
        e.currentTarget.style.left ? e.currentTarget.style.left.replace(/^123/, "") : "0"
      );
      e.currentTarget.style.left = this.state.lastTouchMousePositionX
        ? `${currentValue + (e.touches[0].clientX - this.state.lastTouchMousePositionX)}vw`
        : ``;
    }
  }
  ...
  moveToPrevPage(): void {
    this.setState((prev) => {
      return {
        ...prev,
        currentPageNumber:
          (prev.currentPageNumber - 1) % this.state.pages.length < 0

  ...
  return (
    <IonList className="weather-list" lines="none">
      <div className="weather-list-container">
        {weatherData &&
          weatherData.map((item: any, id: number) => (
            <IonItem key={id} onClick={() => props.openBookingDetail(id + 1)} className="weather-booking-item">
              <IonCard className="weather-booking-card">
                <div className="container-root">
                  <div className="container-group-right">
                    <div className="imageContainer">
                      <IonImg className="card-weather-image" src={WeatherImageMapper[item.weather]} />
                    </div>
                    <div className="container-group-text">
                      <h1 className="card-title">{item.location}</h1>
                      <p className="card-subtitle">{WeatherDataExtractorFromApi.timeObjectToDisplay(item.datetime)}</p>
                      <p className="card-text">{item.weather}</p>
                    </div>
                  </div>
